...
S-25 canonicalizes the default live model to `gemma-4-31b-it`. `TLH_GEMMA_MODEL` remains the env override; route-dry-run reports `live_model` and `model_source`; help/init expose the canonical default; `gemma-3-27b-it` remains only as a historical S-24 stale-default caveat. No live run was executed. The approved baseline remains live-limit 5 with timeout 300s, and the rotating health pool policy is unchanged.
